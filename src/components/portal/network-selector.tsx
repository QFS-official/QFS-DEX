    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <ChevronDown className="ml-1 h-3.5 w-3.5 text-muted-foreground transition-transform group-hover:text-foreground" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align="start"
        className="min-w-[200px] rounded-xl border-white/10 bg-[#1a1832] p-1 text-foreground"
      >
        {label && (
...
function ChainGlyph({ chain }: { chain: ChainConfig }) {
  return (
    <div
      className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-bold text-foreground"
      style={{
